...
@Component({
  selector: 'app-ado-integration-modal',
  templateUrl: './ado-integration-modal.component.html',
  styleUrls: ['./ado-integration-modal.component.scss'],
  standalone: true,
...
export class AdoIntegrationModalComponent implements OnInit {
  isLoading = signal<boolean>(true);
  isLoadingWorkItems = signal<boolean>(false);
  connectionStatus = signal<AdoConnectionStatus>({
    isConnected: false,
  });
  ...
  statusText = computed(() => {
    if (this.isLoading()) return 'Checking ADO integration status...';
    const status = this.connectionStatus();
    return status.isConnected 
      ? 'ADO integration is connected and ready'
      : 'ADO integration is not configured or connection failed';
  });
  ...
  constructor(
    @Inject(MAT_DIALOG_DATA) public data: AdoIntegrationData,
    private dialogRef: MatDialogRef<AdoIntegrationModalComponent>,
  ...
  ngOnInit(): void {
    this.checkAdoIntegrationStatus();
  }

  private async checkAdoIntegrationStatus(): Promise<void> {
    try {
      // Get project metadata to check ADO configuration
      this.store.select(ProjectsState.getMetadata)
        .pipe(take(1))
        .subscribe(async (metadata) => {
          if (!metadata?.integration?.ado) {
            this.connectionStatus.set({
              isConnected: false,
              errorMessage: APP_MESSAGES.ADO_DETAILS_MISSING
            });
            this.isLoading.set(false);
            return;
          }

          const adoConfig = metadata.integration.ado;
          
          try {
            // Validate ADO credentials
            const validationResult = await this.electronService.validateAdoCredentials(
            ...
            this.connectionStatus.set({
              ...
              errorMessage: validationResult.isValid ? undefined : APP_MESSAGES.ADO_INVALID_CREDENTIALS
            });

            // If credentials are valid, configure the ADO service and fetch features
            if (validationResult.isValid) {
              this.isLoading.set(false);
              this.adoService.configure({
                organization: adoConfig.organization!,
            ...
            console.error('Error validating ADO credentials:', error);
            this.connectionStatus.set({
              isConnected: false,
              organization: adoConfig.organization,
              projectName: adoConfig.projectName,
  ...
  async loadFeaturesHierarchy(): Promise<void> {
    try {
      ...
      const featuresHierarchy = await this.adoService.getFeaturesHierarchy();
      ...
      console.error('Error loading feature hierarchy:', error);
      this.toasterService.showError('Failed to load ADO work items');
    } finally {
    ...
    if (status.isConnected) {
      try {
        ...
        if (Object.values(selectedItems).every(array => array.length === 0)) {
          this.toasterService.showWarning('No items selected. Please select at least one item to import.');
          return;
        }
      ...
      this.toasterService.showError('Please configure ADO integration before proceeding');
    }
  }
          ...
          feature.child.forEach(platformFeature => {
            ...
            if (this.isPlatformFeatureSelected(pfId)) {
              ...
              const userStory: Ticket = {
                ...
                specifaiId: platformFeature.specifaiId || `US${platformFeature.pmoId}`,
                ...
                platformFeature.child.forEach(userStory => {
                  ...
                  if (this.isUserStorySelected(usId)) {
                    ...
                    const task: Ticket = {
                      ...
                      specifaiId: userStory.specifaiId || `TASK${userStory.pmoId}`,
                      specifaiType: 'Task', // Map to Task
                      specifaiParentId: platformFeature.specifaiId || `US${platformFeature.pmoId}`, // Link to parent User Story
  ...
  onConfigureIntegration(): void {
    this.dialogRef.close({ configure: true });
  }
   ...
   */
  private cascadeFeatureSelection(featureId: number, isSelected: boolean): void {
    const features = this.featuresWithChildren();
  ...
  togglePlatformFeatureSelection(event: Event, platformFeatureId: number): void {
    // Stop the click event from propagating to parent elements
    event.stopPropagation();
  ...
  private selectParentFeature(platformFeatureId: number): void {
    ...
    for (const feature of features) {
      ...
      const hasPlatformFeature = feature.child.some((pf: Ticket) => parseInt(pf.pmoId) === platformFeatureId);
   ...
   */
  private cascadePlatformFeatureSelection(platformFeatureId: number, isSelected: boolean): void {
    ...
    for (const feature of features) {
      ...
      const pf = feature.child.find((pf: Ticket) => parseInt(pf.pmoId) === platformFeatureId);
      if (pf) {
  ...
  private selectParentHierarchy(userStoryId: number): void {
    ...
    for (const feature of features) {
      ...
      for (const platformFeature of feature.child) {
        ...
        const foundUserStory = platformFeature.child.find((us: Ticket) => parseInt(us.pmoId) === userStoryId);
        if (foundUserStory) {
  ...
  /**
   * Update parent selections based on child selection states
   * This handles both platform features (parent of user stories)
   * and features (parent of platform features)
   */
  /**
   * This method is no longer used since we're now explicitly handling
   * selection in the toggle methods. Keeping as a stub for backward compatibility.
   */
  private updateParentSelections(): void {
    // No longer required as parent selection is handled directly in the toggle methods
    return;
  }
  ...
  private updateAllSelectedState(): void {
    ...
    this.allItemsSelected.set(totalItemCount > 0 && selectedItemCount === totalItemCount);
  }
}
