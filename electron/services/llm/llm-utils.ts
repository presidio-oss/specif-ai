...
export class LLMUtils {
   ...
   */
  static async retrieveKnowledgeBaseContent(
    ...
    try {
      ...
      const references = result.map(
        (item: DocumentInterface) => item.pageContent
      );
      ...
      console.error(
        `Error in retrieveKnowledgeBaseContent: ${
   ...
   */
  static async generateKnowledgeBasePromptConstraint(
    knowledgeBaseId: string,
    prompt: string,
    config: BedrockConfig
  ): Promise<string> {
    ...
    try {
      ...
      const knowledgeBaseContent = await LLMUtils.retrieveKnowledgeBaseContent(
        knowledgeBaseId,
        prompt,
        config
      );
      ...
      console.error(
        `Error in generateKnowledgeBasePromptConstraint: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
      throw error;
  ...
  static async getDocumentEnhancerPrompt(
    type: DbDocumentType = DbDocumentType.BRD,
    mode: PromptMode,
    promptParams: ILLMEnhance
  ) {
    switch (type) {
      ...
      case DbDocumentType.NFR:
        return mode === PromptMode.UPDATE
          ? updateRequirementPrompt(promptParams as IRequirementEnhance)
          : addRequirementPrompt(promptParams as IRequirementEnhance);

      case DbDocumentType.USER_STORY:
        return mode === PromptMode.UPDATE
          ? updateStoryPrompt(promptParams as IStoryEnhance)
          : addUserStoryPrompt(promptParams as IStoryEnhance);

      case DbDocumentType.TASK:
        return mode === PromptMode.UPDATE
      ...
      default:
        console.warn(
          `Unknown document type "${type}", defaulting to requirement prompt.`
        );
        return mode === PromptMode.UPDATE
          ? updateRequirementPrompt(promptParams as IRequirementEnhance)
          : addRequirementPrompt(promptParams as IRequirementEnhance);
